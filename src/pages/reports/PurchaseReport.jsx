...
export default function PurchaseReport() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const result = await reportService.getPurchaseReport(filters)
      setData(result)
      setSummary({ count: result.length, total: result.reduce((s, r) => s + (r.total_amount || 0), 0) })
  ...
  const handleExport = () => {
    exportToCSV(data.map(r => ({
      'Purchase #': r.purchase_number, Date: formatDate(r.purchase_date),
      Supplier: r.suppliers?.name || '—',
      Subtotal: r.subtotal, Tax: r.tax, Discount: r.discount,
      Total: r.total_amount, Status: r.payment_status,
    })), 'purchase_report')
    toast.success('Report exported')
  }

  const columns = [
    { key: 'purchase_number', label: 'Purchase #', render: (v) => <span className="font-mono text-sm">{v}</span> },
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <Breadcrumb items={[{ label: 'Reports' }, { label: 'Purchase Report' }]} />
      <div className="page-header">
        <div><h1 className="page-title">Purchase Report</h1><p className="text-sm text-gray-500 dark:text-gray-400">{data.length} records</p></div>
      ...
      <div className="card p-4">
        <div className="flex flex-wrap gap-4 items-end">
          ...
          <Button variant="primary" onClick={load} loading={loading} icon={TrendingDown}>Generate Report</Button>
        </div>
      </div>

      <div className="card" style={{ padding: 0 }}>
        <DataTable columns={columns} data={data} loading={loading} emptyMessage="No purchase data. Apply filters and generate." emptyIcon={TrendingDown} />
      </div>
    </div>
  )
}
